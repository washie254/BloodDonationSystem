<?php 
	...
	// connect to database
	$db = mysqli_connect('localhost', 'root', '', 'blood_donation_system');
	...
	if (isset($_POST['reg_user'])) {
		...
		if (count($errors) == 0) {
			...
			$query1 = "INSERT INTO userprofile (username, userEmail ) 
	...
	if (isset($_POST['Add_prof'])) {
		...
		if (count($errors) == 0) {
			$query = "UPDATE userprofile
						SET   userID='$userid', firstName = '$fname', lastName ='$lname', telNo ='$phone', bloodType='$bloodtype', rh='$rh', dob='$dob', location='$county'
